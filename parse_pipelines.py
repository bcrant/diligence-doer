...
def parse_pipelines():
    # TODO: Loop through all files here
    ...
    #
    schema_dict = get_schema_names_dict('pipeline_files/example_config.yml')
    lint_yml_for_parser(file)
    replace_yml_env_vars(file, schema_dict)

    with open(file, 'r') as stream:
        schema_name = yaml.safe_load(stream)['name']
        print(schema_name)
    ...
    #
    # print(json.dumps(table_metadata_dict, indent=4, sort_keys=True))
    #
...
def get_schema_names_dict(config_file):
    ...
    with open(config_file, 'r') as c:
        schema_names = yaml.safe_load(c)['default_schema_names']
    return schema_names
    ...
    with open(file_path, 'w') as f_out:
        for line in lines:
            ...
            if '{{ ' not in line:
                ...
                print('bad line: ', line)
                env_var = line.split('{{ ')[1].rsplit(' }}')[0]
                print('env_var: ', env_var)
                if env_var in schema_names_dict.keys():
                    replaced_line = line.replace(
                        '{{ ' + env_var + ' }}',
                        schema_names_dict.get(env_var)
                    )
                    f_out.write(replaced_line)
                    print('replaced line: ', replaced_line)
                else:
